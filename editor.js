    ...
    constructor(pos_x,pos_y,size_x,size_y, num_in_slots = 0, num_out_slots = 0){
        ...
        if(num_in_slots > 0){
            for (let i = 0; i < num_in_slots; i++){
                this.last_slot_id += 1;
                this.slots[this.last_slot_id] = new Slot(0,i * slot_radius * 2, slot_radius);
        ...
        if(num_out_slots > 0){
            for (let i = 0; i < num_out_slots; i++){
                this.last_slot_id += 1;
                this.slots[this.last_slot_id] = new Slot(size_x - (2 * slot_radius),i * slot_radius * 2, slot_radius);
                this.slot_ids.push(this.last_slot_id);
            }
        }
        this.x = pos_x;
...
class Editor {
    ...
    handleMouseMove(event) {
        if (this.is_generating_path) {
            ...
            this.mx = (event.clientX - bounding.left) / this.zoom_scale;
            this.my = (event.clientY - bounding.top) / this.zoom_scale;
            ...
            let bounding = this.canvas.getBoundingClientRect();
            this.nodes[this.currently_dragging]['x'] = (event.clientX - bounding.left) / this.zoom_scale - this.drag_offset_x;
            this.nodes[this.currently_dragging]['y'] = (event.clientY - bounding.top) / this.zoom_scale - this.drag_offset_y;
        } else if (this.dragging_graph) {
            let bounding = this.canvas.getBoundingClientRect();
            let mx = (event.clientX - bounding.left) / this.zoom_scale;
            let my = (event.clientY - bounding.top) / this.zoom_scale;
    ...
    handleMouseUp(event) {
        ...
        if(this.is_generating_path){
            ...
            if(this.target_highlighted >= 0 && this.target_highlighted_slot >= 0 &&
                ...
                this.last_path_id += 1;
                this.paths[this.last_path_id] = new Path(this.current_highlighted, this.current_highlighted_slot, this.target_highlighted, this.target_highlighted_slot);
    ...
    handleWheel(event) {
        let zoom_level_jump = event.deltaY / -400;
        let target_zoom_scale = 1 / Math.min(Math.max((1 / this.zoom_scale) + zoom_level_jump , this.MIN_ZOOM), this.MAX_ZOOM);
        let bounding = this.canvas.getBoundingClientRect();
        let m_canvas_x = event.clientX - bounding.left;
        let m_canvas_y = event.clientY - bounding.top;
        let mx = (m_canvas_x / this.zoom_scale) - this.graph_offset_x;
        let my = (m_canvas_y / this.zoom_scale) - this.graph_offset_y;
    ...
    registerGlobalRedraw(fn){
        this.global_redraw = fn;
    }
    ...
    addNode(pos_x = 0,pos_y = 0,size_x = 100,size_y = 100, num_in_slots = 0, num_out_slots = 0) {
        ...
        this.nodes[this.last_node_id] = new EditorNode(pos_x,pos_y,size_x,size_y, num_in_slots, num_out_slots);
    }

    // registerRedraw(key_path,fn) {
    //     // Handle Nesting here, also handle removal for nested
    //     // let key_id = getKeyId(key_path);
    //     let key_id = key_path;
    //     this.redraws[key_id] = fn;
    // }
}